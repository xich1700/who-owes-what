...
**Live demo: [who-owes-what.streamlit.app](https://who-owes-what.streamlit.app)**
(runs the full Tier 4 version; note the demo's database can reset on
redeploys, so don't rely on it for anything long-term - clone and run
locally for persistent data)
...
Frontend only - no backend. All four tiers are built and run as a

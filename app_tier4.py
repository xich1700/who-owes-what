...
Tier 4 (Bonuses): multi-currency groups, recurring/repeatable expenses,
copy-paste chat summary, polite reminder messages, and AI-powered
receipt scanning (needs your own Anthropic API key) - on top of
everything from Tiers 1-3 (repayment plans, unequal shares,
natural-language entry, closing a settled group).

Run with:  streamlit run app.py
Requires:  pip install streamlit
Optional (for receipt scanning only):  pip install anthropic
"""
...
def init_db():
    ...
    if "currency" not in group_cols:
        conn.execute("ALTER TABLE groups ADD COLUMN currency TEXT NOT NULL DEFAULT 'EUR'")

    expense_cols = [r["name"] for r in conn.execute("PRAGMA table_info(expenses)").fetchall()]
    if "is_recurring" not in expense_cols:
        conn.execute("ALTER TABLE expenses ADD COLUMN is_recurring INTEGER NOT NULL DEFAULT 0")
...
# ---- Group helpers ----
def create_group(manager_id: str, name: str, currency: str = "EUR") -> str:
    conn = get_conn()
    group_id = uuid.uuid4().hex
    share_token = secrets.token_urlsafe(10)
    conn.execute(
        "INSERT INTO groups (id, manager_id, name, share_token, closed, currency) VALUES (?, ?, ?, ?, 0, ?)",
        (group_id, manager_id, name, share_token, currency),
    )
    conn.commit()
    conn.close()
    return group_id
...
def add_expense(group_id: str, payer_id: str, amount: float, label: str, beneficiary_weights: dict, is_recurring: bool = False):
    conn = get_conn()
    expense_id = uuid.uuid4().hex
    conn.execute(
        "INSERT INTO expenses (id, group_id, payer_id, amount, label, is_recurring) VALUES (?, ?, ?, ?, ?, ?)",
        (expense_id, group_id, payer_id, amount, label, int(is_recurring)),
    )
    conn.executemany(
...
def update_expense(expense_id: str, payer_id: str, amount: float, label: str, beneficiary_weights: dict, is_recurring: bool = False):
    conn = get_conn()
    conn.execute(
        "UPDATE expenses SET payer_id = ?, amount = ?, label = ?, is_recurring = ? WHERE id = ?",
        (payer_id, amount, label, int(is_recurring), expense_id),
    )
...
def duplicate_expense(group_id: str, source_expense: dict):
    """Used by the 'Repeat' button on a recurring expense - same payer, amount,
    label and split, logged again right now."""
    return add_expense(
        group_id,
        source_expense["payer_id"],
        source_expense["amount"],
        source_expense["label"],
        source_expense["beneficiaries"],
        is_recurring=True,
    )
...
# =========================================================
# 4) SHARE-READY TEXT (summary + reminder)
# =========================================================
def build_summary_text(group, people, outstanding, transactions, currency, total_spent):
    lines = [f"💸 {group['name']} — summary", f"Total spent: {fmt_money(total_spent, currency)}"]
    lines.append("")
    for p in people:
        b = outstanding[p["id"]]
        if b > 0.005:
            lines.append(f"🟢 {p['name']} is owed {fmt_money(b, currency)}")
        elif b < -0.005:
            lines.append(f"🔴 {p['name']} owes {fmt_money(abs(b), currency)}")
        else:
            lines.append(f"⚪ {p['name']} is settled up")
    lines.append("")
    if transactions:
        lines.append("To settle up:")
        for t in transactions:
            lines.append(f"- {t['from_name']} → {t['to_name']}: {fmt_money(t['amount'], currency)}")
    else:
        lines.append("Everyone is settled up ✅")
    return "\n".join(lines)


def build_reminder_text(debtor_name: str, group_name: str, transactions: list, currency: str):
    owed_to = [t for t in transactions if t["from_name"] == debtor_name]
    if not owed_to:
        return f"{debtor_name} doesn't owe anything in {group_name} right now."
    if len(owed_to) == 1:
        t = owed_to[0]
        detail = f"{fmt_money(t['amount'], currency)} to {t['to_name']}"
    else:
        detail = ", ".join(f"{fmt_money(t['amount'], currency)} to {t['to_name']}" for t in owed_to)
    return (
        f"Hey {debtor_name}! 👋 Just a friendly nudge about {group_name} — "
        f"looks like you still owe {detail}. No rush, just didn't want it to slip through the cracks. Thanks!"
    )


# =========================================================
# 5) RECEIPT SCANNING (optional - needs the user's own Anthropic API key)
# =========================================================
def extract_receipt_via_ai(image_bytes: bytes, media_type: str, api_key: str):
    """
    Returns (result_dict, error_message). result_dict has "amount" and "label"
    when the model could read the receipt. Never touches the database or
    creates an expense directly - the caller always routes this through the
    normal Add Expense confirmation screen.
    """
    try:
        import anthropic
    except ImportError:
...
def render_home():
    ...
    if not groups:
        ...
        for g in groups:
            ...
            col1, col2 = st.columns([4, 1])
            with col1:
                st.markdown(f"**{g['name']}**  \n{people_count} people · {expense_count} expenses · {g['currency']}{status}")
            with col2:
...
def render_create_group():
    ...
    if st.button("Create group", type="primary", disabled=not can_create, use_container_width=True):
        group_id = create_group(st.session_state.manager_id, group_name.strip(), currency)
...
def render_group_detail():
    ...
    with col1:
        ...
        st.caption((", ".join(p["name"] for p in people) if people else "No one in this group yet.") + f" · {currency}")
    with col2:
        if st.button("← Groups"):
            go("home")
    ...
    with tab_expenses:
        if not is_closed and len(people) >= 2:
            ...
            with st.expander("📷 Scan a receipt"):
                st.caption(
                    "Reads the total and merchant off a photo using your own Anthropic API key. "
                    "The key is only used for this request — it's never saved to disk or the database."
                )
                receipt_file = st.file_uploader("Receipt photo", type=["jpg", "jpeg", "png"], key="receipt_upload")
                api_key = st.text_input(
                    "Your Anthropic API key", type="password", key="anthropic_api_key",
                    help="Get one at console.anthropic.com. Kept only in this browser session.",
                )
                if st.button("Scan receipt"):
                    if not receipt_file:
                        ...
                        media_type = "image/png" if receipt_file.type == "image/png" else "image/jpeg"
                        with st.spinner("Reading the receipt…"):
                            result, error = extract_receipt_via_ai(receipt_file.getvalue(), media_type, api_key.strip())
                        if error:
                            st.error(error)
                        else:
                            st.session_state.prefill_expense = {
                                "ok": True,
                                "payer_id": people[0]["id"],
                                "payer_name": people[0]["name"],
                                "amount": result["amount"],
                                "label": result["label"],
                                "beneficiary_ids": [p["id"] for p in people],
                            }
                            go("add_expense", editing_expense_id=None)
        ...
        if len(people) < 2:
            ...
            for exp in expenses:
                ...
                recurring_tag = " 🔁" if exp["is_recurring"] else ""
                with st.container(border=True):
                    c1, c2 = st.columns([4, 1])
                    with c1:
                        st.markdown(f"**{label}**{recurring_tag} — {fmt_money(exp['amount'], currency)}")
                        st.caption(f"Paid by {payer_name} · for {benef_desc}")
                    with c2:
                        if not is_closed:
                            if exp["is_recurring"]:
                                if st.button("🔁 Repeat", key=f"repeat_{exp['id']}"):
                                    duplicate_expense(group["id"], exp)
                                    st.rerun()
    ...
    # ---- Totals tab ----
    with tab_totals:
        st.metric("Total spent by the group", fmt_money(total_spent, currency))
        ...
        if not people:
            ...
            for p in people:
                ...
                b = outstanding[pid]
                if b > 0.005:
                    status = f"🟢 Is owed {fmt_money(b, currency)}"
                elif b < -0.005:
                    status = f"🔴 Owes {fmt_money(abs(b), currency)}"
                else:
                    status = "⚪ Settled up"
                note = ""
                if round(expense_balance[pid], 2) != b:
                    note = f"  \n<sub>from expenses: {fmt_money(expense_balance[pid], currency)} · adjusted for repayments below</sub>"
                st.markdown(
                    f"**{p['name']}** — paid {fmt_money(paid[pid], currency)} · share is {fmt_money(share[pid], currency)}  \n{status}{note}",
                    unsafe_allow_html=True,
                )

            st.divider()
            st.subheader("📋 Copy-paste summary")
            st.caption("Ready to drop straight into the group chat.")
            summary_text = build_summary_text(group, people, outstanding, transactions, currency, total_spent)
    ...
    with tab_settle:
        ...
        if not transactions:
            ...
            for t in transactions:
                c1, c2 = st.columns([4, 1])
                with c1:
                    st.markdown(f"- **{t['from_name']}** gives **{t['to_name']}** {fmt_money(t['amount'], currency)}")
                with c2:
                    if not is_closed:
        ...
        if not is_closed:
            ...
            with st.form("manual_repayment_form", clear_on_submit=True):
                ...
                r_amount = st.number_input(f"Amount ({currency})", min_value=0.01, step=0.5, format="%.2f")
            ...
            for r in repayments:
                ...
                with c1:
                    st.markdown(
                        ...
                        f"{name_by_id.get(r['to_person_id'], '?')}: {fmt_money(r['amount'], currency)}"
                    )
                with c2:
                    if not is_closed:
        ...
        if transactions:
            st.divider()
            st.subheader("✉️ Send a reminder")
            debtor_names = sorted(set(t["from_name"] for t in transactions))
            chosen = st.selectbox("Who still owes money?", debtor_names, key="reminder_debtor")
            reminder_text = build_reminder_text(chosen, group["name"], transactions, currency)
...
def render_add_expense():
    ...
    if existing:
        ...
        default_amount = 0.01
    amount = st.number_input(f"Amount ({group['currency']})", min_value=0.01, step=0.5, format="%.2f", value=default_amount)
    ...
    default_recurring = existing["is_recurring"] if existing else False
    is_recurring = st.checkbox("🔁 Mark as recurring (e.g. rent, weekly shop) so it can be repeated with one click later", value=default_recurring)
    ...
    with col3:
        if st.button("Save", type="primary", disabled=bool(error), use_container_width=True):
            ...
            if existing:
                update_expense(existing["id"], payer_id, round(amount, 2), label.strip(), beneficiary_weights, is_recurring)
            else:
                add_expense(group["id"], payer_id, round(amount, 2), label.strip(), beneficiary_weights, is_recurring)
            go("group_detail", editing_expense_id=None, prefill_expense=None)
